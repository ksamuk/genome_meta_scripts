...
# list of files 
file.list <- list.files("snp_tables", pattern="*\\.tab", full.names = TRUE)

# make output dirs
dir.create("snp_tables/merged_chromo", showWarnings = FALSE)
...
for (i in 1:length(chrom.list)){
  ...
  write.table(file.merged.chrom,file=file.path("snp_tables/merged_chromo",out.file.name),quote=FALSE,row.names=FALSE)
...
chromo.files <- list.files("snp_tables/merged_chromo")
...
for (i in 1:length(chromo.files)){
  file.current <- fread(file.path("snp_tables/merged_chromo",chromo.files[i]),header=TRUE,colClasses="character")
  header.list[[i]] <- names(file.current)
  header.tf <- i==1
  print(paste("Concatenating chromo file",i,"..."))
  write.table(file.current,
              file=file.path("snp_tables/merged_chromo", merged.file.name),
              col.names=header.tf,
              append=!header.tf,
              row.names=FALSE,
              quote=FALSE,

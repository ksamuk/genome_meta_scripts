...
prop.clustered <- cluster.df %>%
  ...
  filter(num.lg > 1) %>%
    ggplot(aes(x = group2.new, y = more.clustered))+
			...
			geom_errorbar(stat = "hline", 
										...
										aes(ymax = ..y.. , ymin = ..y..),
										color = pal)+
			theme_hc(base_size = size) +
			theme(axis.title.y = element_text(vjust=3),
						axis.text.x = element_blank(),
						plot.margin = rep(unit(1,"cm"),4))+
		ylab("Prop. chromosomes clustered")+
		theme.all+
		xlab(NULL)

grid.arrange(prop.clustered, nnd.diff, coeff.dispersion, nnd.rep)
...
coeff.dispersion <- cluster.df %>%
  ...
  summarise(disp.out = mean(disp.out))%>%
	filter(!is.na(disp.out)) %>%
		ggplot(aes(x = group2.new, y = disp.out))+
		geom_point(position = position_jitter(width = 0.3), size = 1)+
		geom_errorbar(stat = "hline", 
									yintercept = "mean",
									width = 0.8, size = 2, 

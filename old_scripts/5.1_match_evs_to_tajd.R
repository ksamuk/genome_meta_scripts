...
#tajd file
stats.file <- file.path(getwd(), "ev_prep_scripts/tajimasD/tajimasD.combined.D")

#the "stats" file, specifcally an "outlier" file
stats.file <- read.table(file = stats.file,header=TRUE)
...
for (i in 1:length(ev.files)){
  ...
  ###loop through lgs, matching evs as we go
  ...
  names(matched.evs)[9] <- sapply(strsplit(ev.files[i], split=".txt"), function(x)x[1])
...
file.name<-paste("tajimasD.master",date.stamp,".txt",sep="")

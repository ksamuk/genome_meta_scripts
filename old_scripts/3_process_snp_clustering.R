...
calculate_nndist_all_lg <- function (stats.file) {
  ...
  for (j in unique(stats.file$lg)){
    ...
    if (num.outliers > 1){
      ...
      null.mean.nnds <- replicate(10000,calculate.null.nnd(stats.file.lg, num.outliers))
...
mclapply(stats.files, calculate_coeff_dispersion_stats_file, mc.cores = 8, mc.silent = FALSE) 
